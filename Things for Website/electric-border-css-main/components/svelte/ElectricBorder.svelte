<script lang="ts">
  /**
   * Electric Border Component for Svelte
  ...
  export let preset: 'cyan' | 'purple' | 'pink' | 'green' | 'orange' | 'red' | 'blue' | 'yellow' = 'cyan';
  ...
  $: borderColor = color || colorPresets[preset];
  $: containerStyle = `
...
<!-- Card Container -->
<div
  class="electric-card-container {customClass}"
  ...
  <div class="electric-content-container">
    <slot>
      <div class="electric-content-top">
        {#if label}
          <div class="electric-scrollbar-glass">{label}</div>
        {/if}
        {#if title}
          <p class="electric-title">{title}</p>
        {/if}
      </div>
      <hr class="electric-divider" />
      <div class="electric-content-bottom">
        {#if description}
          <p class="electric-description">{description}</p>
        {/if}
      </div>
    </slot>
  </div>
...
<style>
  ...
  .electric-main-card {
    ...
    margin-top: -4px;
    margin-left: -4px;
    filter: url(#turbulent-displace-svelte);
    transition: 0.3s;
  }
