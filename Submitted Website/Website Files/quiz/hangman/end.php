...
<div class="page-wrap">
  ...
  <?php else:
    ...
    foreach ($attempts as $i => $a):
      ...
      $cPct = $det['correctPct'] ?? ($totalWords>0?round($a['score']/$totalWords*100):0);
    ...
    <div class="attempt-card" style="animation-delay:<?= $i*0.1 ?>s">
      ...
      <div class="attempt-body">
        <table>
          ...
          <tr><td class="td-key">Words Guessed</td><td class="td-val"><?= (int)$a['score'] ?> / <?= $totalWords ?></td></tr>
          <tr><td class="td-key">Grade</td><td class="td-val"><?= (int)$a['score'] ?>.00 out of <?= $totalWords ?>.00 <strong>(<?= $cPct ?>%)</strong>
  ...
  <div class="nav-row">
    ...
    <?php if ($canRetry): ?><a href="<?= SITE_ROOT ?>quiz/hangman/start.php?quiz_id=<?= $quizId ?>" class="btn btn-retry">Retry →</a><?php endif; ?>
  </div>
</div>
</body>
</html>
