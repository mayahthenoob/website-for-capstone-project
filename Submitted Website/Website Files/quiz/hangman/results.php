...
$attemptsJson = json_encode(array_map(function($a){$d=json_decode($a['attempt_data']??'{}',true)??[];return array_merge($d,['score'=>$a['score'],'max_score'=>$a['max_score'],'time_taken'=>$a['time_taken']]);}, $attempts));
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
body{font-family:'Syne',sans-serif;background:radial-gradient(ellipse at top,#0d0630,#020008 70%);min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px 20px;}
...
.fill-correct{stroke:url(#gc);}.fill-speed{stroke:url(#gs);}.fill-streak{stroke:url(#gst);}
...
.circle-correct .circle-pct{color:var(--green);}.circle-streak .circle-pct{color:var(--gold);}
...
.btn{flex:1;padding:20px;background:none;border:none;color:rgba(255,255,255,0.7);font-family:'Syne',sans-serif;font-weight:700;font-size:0.88rem;cursor:pointer;transition:0.2s;text-decoration:none;display:flex;align-items:center;justify-content:center;}
...
<div class="game-card">
  ...
  <svg width="0" height="0"><defs>
    <linearGradient id="gc" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#5effa3"/><stop offset="100%" stop-color="#00c896"/></linearGradient>
    <linearGradient id="gs" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#00d2ff"/><stop offset="100%" stop-color="#3081d0"/></linearGradient>
    <linearGradient id="gst" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#f1c40f"/><stop offset="100%" stop-color="#e67e22"/></linearGradient>
  </defs></svg>
  ...
  <div class="inner-frame">
    <div class="circles-row">
      <div class="circle-item circle-correct"><div class="circle-svg-wrap"><svg viewBox="0 0 130 130"><circle class="track" cx="65" cy="65" r="55"/><circle class="fill fill-correct" id="fillCorrect" cx="65" cy="65" r="55"/></svg><div class="circle-inner"><span class="circle-pct" id="pctCorrect">0%</span></div></div><div class="circle-label">Correct</div></div>
      <div class="circle-item"><div class="circle-svg-wrap"><svg viewBox="0 0 130 130"><circle class="track" cx="65" cy="65" r="55"/><circle class="fill fill-speed" id="fillSpeed" cx="65" cy="65" r="55"/></svg><div class="circle-inner"><span class="circle-pct" id="pctSpeed">0%</span></div></div><div class="circle-label">Speed</div></div>
      <div class="circle-item circle-streak"><div class="circle-svg-wrap"><svg viewBox="0 0 130 130"><circle class="track" cx="65" cy="65" r="55"/><circle class="fill fill-streak" id="fillStreak" cx="65" cy="65" r="55"/></svg><div class="circle-inner"><span class="circle-pct" id="pctStreak">0%</span></div></div><div class="circle-label">Streak</div></div>
    </div>
    <hr class="divider">
    <div class="stats-grid">
      <div class="stat-box"><span class="stat-val" id="statWords">—</span><span class="stat-key">Words Guessed</span></div>
      <div class="stat-box"><span class="stat-val" id="statTime">—</span><span class="stat-key">Time Taken</span></div>
    </div>
  </div>
  <?php else: ?><p style="color:rgba(255,255,255,0.6);text-align:center;padding:40px 0">No attempts yet.</p><?php endif; ?>
  <div class="nav-buttons">
    <a href="<?= SITE_ROOT ?>class.php?id=<?= $quiz['class_id'] ?>" class="btn">← Back</a>
    <a href="<?= SITE_ROOT ?>quiz/hangman/end.php?quiz_id=<?= $quizId ?>" class="btn">Details →</a>
  </div>
</div>
<script>
...
if (attempt) {
  ...
  const { correctPct=0, timePct=0, streakPct=0, won=0, total=0, durationSecs=0 } = attempt;
  const mins = Math.floor(durationSecs / 60), secs = durationSecs % 60;
  document.getElementById("statWords").textContent = `${won || attempt.score || 0} / ${total || attempt.max_score || 0}`;
  document.getElementById("statTime").textContent = `${mins}m ${secs}s`;
  const C = 2 * Math.PI * 55;
  function anim(fillId, el, pct, delay) { setTimeout(() => { document.getElementById(fillId).style.strokeDashoffset = C-(pct/100)*C; let cur=0,step=pct/60; const iv=setInterval(()=>{cur=Math.min(cur+step,pct);el.textContent=Math.round(cur)+"%";if(cur>=pct)clearInterval(iv);},20); }, delay); }
  anim("fillCorrect", document.getElementById("pctCorrect"), correctPct, 300);
  anim("fillSpeed",   document.getElementById("pctSpeed"),   timePct||0, 500);
  anim("fillStreak",  document.getElementById("pctStreak"),  streakPct||0, 700);
}
</script>
</body>
</html>
