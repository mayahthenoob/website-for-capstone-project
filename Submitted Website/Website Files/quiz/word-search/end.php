...
$totalWords = count($quizData['words'] ?? []);
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Quiz Details — <?= h($quiz['title']) ?></title>
...
.attempt-card:nth-child(2){animation-delay:0.1s;}
.attempt-card:nth-child(3){animation-delay:0.2s;}
...
.ms-correct{color:#5effa3;}
.ms-speed{color:#00d2ff;}
.ms-streak{color:#f1c40f;}
...
.btn-retry{background:linear-gradient(135deg,var(--brand),var(--brand2));color:#fff;box-shadow:0 4px 20px rgba(240,78,132,0.35);}
.btn-retry:hover{transform:translateY(-1px);}
.no-attempts{color:rgba(255,255,255,0.4);text-align:center;padding:40px;}
</style>
</head>
<body>
<div class="page-wrap">
  ...
  <?php if (empty($attempts)): ?>
    <div class="no-attempts">No attempts recorded yet.</div>
  <?php else:
    $bestIdx = 0;
    ...
    foreach ($attempts as $i => $a):
      ...
      $correctPct = isset($details['correctPct']) ? $details['correctPct'] : (($totalWords > 0) ? round($a['score'] / $totalWords * 100) : 0);
      $timePct    = $details['timePct'] ?? 0;
      $streakPct  = $details['streakPct'] ?? 0;
      $isBest     = count($attempts) > 1 && $i === $bestIdx;
      $durSecs    = $a['time_taken'] ?? 0;
      $mins = floor($durSecs / 60); $secs = $durSecs % 60;
  ?>
    <div class="attempt-card">
      ...
      <div class="attempt-body">
        <table>
          ...
          <tr><td class="td-key">Words Found</td><td class="td-val"><?= (int)$a['score'] ?> / <?= $totalWords ?></td></tr>
          <tr><td class="td-key">Grade</td><td class="td-val"><?= (int)$a['score'] ?>.00 out of <?= $totalWords ?>.00 <strong>(<?= $correctPct ?>%)</strong>
            <div class="score-bar-wrap"><div class="score-bar-fill" style="width:<?= $correctPct ?>%"></div></div>
          </td></tr>
        </table>
      </div>
      <div class="mini-scores">
        <div class="mini-score"><span class="mini-score-val ms-correct"><?= $correctPct ?>%</span><span class="mini-score-key">Correct</span></div>
        <div class="mini-score"><span class="mini-score-val ms-speed"><?= $timePct ?>%</span><span class="mini-score-key">Speed</span></div>
        <div class="mini-score"><span class="mini-score-val ms-streak"><?= $streakPct ?>%</span><span class="mini-score-key">Streak</span></div>
      </div>
  ...
  <div class="nav-row">
    ...
    <?php if ($canRetry): ?>
      <a href="<?= SITE_ROOT ?>quiz/word-search/start.php?quiz_id=<?= $quizId ?>" class="btn btn-retry">Retry →</a>
    <?php endif; ?>
  </div>
</div>
</body>
