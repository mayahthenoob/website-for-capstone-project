...
header { background:linear-gradient(135deg,#1a0a2e,#2d1354); padding:0 32px; height:64px; display:flex; align-items:center; gap:12px; box-shadow:0 2px 20px rgba(124,58,145,0.3); }
...
.info-card { background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:28px 32px;margin-bottom:16px;box-shadow:0 2px 16px rgba(124,58,145,0.05); }
...
.info-row { display:flex;align-items:baseline;gap:10px;padding:9px 0;border-bottom:1px solid var(--bg);font-size:0.95rem; }
.info-row:last-child { border-bottom:none; }
.info-key { color:var(--muted);font-size:0.88rem;min-width:160px;flex-shrink:0; }
...
<div class="container">
  ...
  <h1><?= h($quiz['title']) ?></h1>
  <div class="warning-box"><?= h($quiz['description'] ?: 'Find all the hidden words in the grid before the time runs out!') ?></div>
  ...
  <div class="info-card">
    ...
    <div class="info-row"><span class="info-key">Type</span><span class="info-val">Word Search Puzzle</span></div>
    <div class="info-row"><span class="info-key">Words to Find</span><span class="info-val"><?= count($quizData['words'] ?? []) ?></span></div>
    ...
    <div class="info-row"><span class="info-key">Attempts Allowed</span><span class="info-val"><?= $quiz['attempts_allowed'] ?></span></div>
    <div class="info-row"><span class="info-key">Your Attempts Left</span><span class="info-val"><?= $attLeft ?></span></div>
  </div>

  <?php if ($attLeft <= 0): ?>
    <div class="no-attempts">You have used all your attempts for this quiz.</div>
    <div class="actions"><a href="<?= SITE_ROOT ?>quiz/word-search/results.php?quiz_id=<?= $quizId ?>" class="btn btn-primary">View Results</a></div>
  <?php else: ?>
    <div class="actions">
      <a href="<?= SITE_ROOT ?>class.php?id=<?= $quiz['class_id'] ?>" class="btn btn-secondary"><i class='bx bx-arrow-back'></i> Back</a>
      <a href="<?= SITE_ROOT ?>quiz/word-search/play.php?quiz_id=<?= $quizId ?>" class="btn btn-primary"><i class='bx bx-play'></i> Start Quiz</a>
    </div>
  <?php endif; ?>
</div>
</body>
